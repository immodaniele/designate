...
class NovaCustomHandler(BaseAddressHandler):
    ...
    def process_notification(self, context, event_type, payload):
        LOG.debug('NovaCustomHandler received notification - %s', event_type)
        ...
        if not project_id:
            LOG.error('NovaCustomHandler: project_id is None, ignore the event.')
            return
        ...
        if not subzone:
            LOG.error(f'NovaCustomHandler: No zone found for project {project_id}, ignore the event.')
            return
